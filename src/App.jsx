...
export default function App() {
  ...
  const handleAdminSave = () => {
    ...
    saveEvents(updated);
    setModalView(null);
    showToast(adminForm.type === "free" ? "Datum freigegeben" : "Datum gespeichert", `${fmtDate(selectedDate)}${adminForm.label ? " · " + adminForm.label : ""}`, true);
  };
  ...
  const handleCustomerSubmit = () => {
    ...
    setSubmitAttempted(false);
    setModalView(null);
    showToast("Anfrage gesendet! Wir melden uns bald bei Ihnen.");
  };
                    ...
                    <>
                      ...
                      <div style={{ background:`${BRAND.moosgruen}08`, border:`1px solid ${BRAND.moosgruen}20`, borderRadius:10, padding:"10px 14px", marginBottom:10 }}>
                        ...
                        <div style={{ display:"flex", gap:8 }}>
                          <div style={{ flex:1 }}>
                            <label style={{ fontSize:10, color:"#999", fontWeight:600 }}>Kuchen (à € 4,50)</label>
                            <input type="number" min="0" placeholder="0" value={formData.cakeCount||""} onChange={e => setFormData(f=>({...f, cakeCount:e.target.value}))} style={{ ...inputStyle, marginBottom:0 }} />
                          </div>
                          <div style={{ flex:1 }}>
                            <label style={{ fontSize:10, color:"#999", fontWeight:600 }}>Kaffee (à € 3,10)</label>
                            <input type="number" min="0" placeholder="0" value={formData.coffeeCount||""} onChange={e => setFormData(f=>({...f, coffeeCount:e.target.value}))} style={{ ...inputStyle, marginBottom:0 }} />
                          </div>
                        </div>
                      </div>
